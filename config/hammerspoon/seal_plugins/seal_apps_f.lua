...
function obj.completionCallback(rowInfo)
  ...
  if rowInfo["type"] == "launchOrFocus" then
    ...
    -- custom handlers for apps I want to always open a new window, instead of focus the current existing window
    if rowInfo["path"] == "/Applications/Google Chrome.app" then
      hs.task
          .new(rowInfo["path"] .. "/Contents/MacOS/Google Chrome", nil, { "--new-window", "www.google.com" })
          :start()
    elseif rowInfo["path"] == "/Applications/kitty.app" then
      -- hs.task.new("/opt/homebrew/bin/kitty", nil, { "--single-instance", "&" }):start()
      print("kitty nohup")
      os.execute("nohup /opt/homebrew/bin/kitty --single-instance &")
    end
  elseif rowInfo["type"] == "kill" then
    hs.application.get(rowInfo["pid"]):kill()
  elseif rowInfo["type"] == "reveal" then
    hs.osascript.applescript(
      string.format([[tell application "Finder" to reveal (POSIX file "%s")]], rowInfo["path"])
    )
