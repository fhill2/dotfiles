...
---             textFont='SF Mono'
---             textSize=20}
obj.helperFormat = { atScreenEdge = 2, strokeColor = { white = 0, alpha = 2 }, textFont = "Courier", textSize = 20 }
...
function obj.showHelper(keyFuncNameTable)
...
local function onHyperHold(keycode, timeoutMs, onHold, onRelease)
  local state = {
    ...
    onHold = onHold,
    onRelease = onRelease,
    lastKey = 0, -- track the last key, because hammerspoon registers key repeat for the key, even though karabiner repeat=false
  }
  ...
  state.downTap = hs.eventtap.new({
    hs.eventtap.event.types.keyDown,
  }, function(event)
    local event_keycode = event:getKeyCode()
    local key_is_hyper = event_keycode == keycode
  ...
  state.upTap = hs.eventtap.new({
    hs.eventtap.event.types.keyUp,
  }, function(event)
    local key_is_hyper = event:getKeyCode() == keycode
    if key_is_hyper then
      state.held = false
      state.lastKey = 0
      onRelease()
      cancelTimer()
