...
# XID = tabbed x window id
# tabbed is started and tabbed returned its XID after creation
# this is sent to main process, and used for every invocation of zathura and mpv to embed into the tabbed xid

term_nuke () {
    # $1 -> $XID, $2 -> $FILE
    # $TERMINAL "$1" -e "$NUKE" "$2" &
    KITTY_XID=$(i3_open_wait.py kitty)
    notify-send "$XID"
    xdotool reparentwindow $KITTY_XID $XID
}
