...
utils.get_editor_wins_no_previewer = function()
  ...
  if not picker then return editor_wins end
  local wins = {}
  for _, win in ipairs(editor_wins) do
    if picker.preview_win ~= win then table.insert(wins, win) end
  end
  return wins
end

utils.find_prompt = function()
  return vim.tbl_filter(function(b) return vim.bo[b].filetype == "TelescopePrompt" end, vim.api.nvim_list_bufs())[1]
end
