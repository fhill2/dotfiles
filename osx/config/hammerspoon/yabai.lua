-- https://github.com/Elido/elido-hotkeys/blob/main/src/yabai.lua
-- local json = require("cjson")

function focusOnAnyVisibleWindow()
  -- Focus any window that is current visible
  execTaskInShellSync(
    [[yabai -m window --focus $(yabai -m query --windows | jq -rj '. | map(select(.["is-visible"] == true)) | .[0].id')]]
  )
end

function getFocusedWindowId()
  return execTaskInShellSync(
    "yabai -m query --windows | jq -rj '. | map(select(.[\"has-focus\"] == true)) | .[0].id'"
  )
end
...
function getFocusedSpace()
  ...
  if win ~= nil then
    ...
    return hs.json.decode(execTaskInShellSync("yabai -m query --spaces --space " .. spaceIndex))
  else
    -- if there's no window just get the space that says it's in focus
    return hs.json.decode(
      execTaskInShellSync("yabai -m query --spaces | jq -rj '. | map(select(.[\"has-focus\"] == true)) | .[0]'")
    )
  end
end
...
function getFocusedWindow()
  local winStr =
      execTaskInShellSync("yabai -m query --windows | jq -rj '. | map(select(.[\"has-focus\"] == true)) | .[0]'")
...
function getSortedDisplays()
  ...
  local tab = hs.json.decode(displaysStr)
...
function moveWindowToSpaceWithinDisplay(space_sel)
  ...
  local display = js.json.decode(
    execTaskInShellSync(
      [[yabai -m query --displays | jq -rj ". | map(select(.[\"index\"] == ]]
      .. currentDisplayIndex
      .. [[)) | .[0]"]]
    )
  )
...
function gotoDisplay(display_sel)
  local win = getFocusedWindow()
  local supportedSel = display_sel == "next"
      or display_sel == "prev"
      or display_sel == "east"
      or display_sel == "west"
...
function gotoSpace(space_sel, withinDisplay)
  if space_sel == "next" or space_sel == "prev" or space_sel == "east" or space_sel == "west" then
    ...
    if withinDisplay == true then
      ...
      spaces = hs.json.decode(execTaskInShellSync("yabai -m query --spaces"))
    end
